...
- (void)setupSubViews
{
    ...
    if (self.videoModel.isAlreadyDownload)
    {
        ...
        if ([file_manager fileExistsAtPath:playUrl])
        {
            AndyLog(@"File exists");
            isLocalExist = YES;
...
- (void)setupNoticeLabel
{
    ...
    noticeLabel.text = @"Video loading failed, please check the network connection.";
...
- (void)dealloc
{
    ...
    AndyLog(@"The video play page memory has been released.");
}
