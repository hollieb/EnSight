...
- (void)setupSubViews
{
    ...
    [favoriteButton setTitle:@"Collection" forState:UIControlStateNormal];
    [favoriteButton setTitle:@"Collected" forState:UIControlStateSelected];
...
//    self.descriptionLabel.frame = (CGRect){{descriptionLabelX, descriptionLabelY}, descriptionLabelSize};
    ...
    AndyLog(@"I haha ​​ha%d", videoModel.isAlreadyDownload);
    ...
    else
    {
        [AndyFMDBTool deleteBySingle:self.videoModel isFavorite:YES  success:^{
            ...
            if (favoriteModel != nil)
            {
                ...
                AndyLog(@"favoriteModel Has been found");
            }
...
- (void)downloadButtonClick:(AndyVideoDetailOperateButton *)button
{
    if (button.selected == NO && !self.videoModel.isAlreadyDownload && !self.videoModel.isDownloading)
    {
        if ([AndyCommonFunction isNetworkConnected])
        {
            ...
            {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Tips" message:@"Currently on non-WiFi networks, continuing to download will consume your data. Are you sure you want to download?" delegate:self cancelButtonTitle:@"Csncel" otherButtonTitles:@"Continue", nil];
                [alertView show];
            }
        }
        else
    ...
    {
        AndyLog(@"While clicking again to download, ignored");
    }
}
