...
- (void)onPan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (isVerticalMoving)
    {
        if (self.isTouchLeft)
        {
            ...
            {
                [self animateBrightUIViewShow];
                AndyLog(@"Perform the brightness prompt display again");
            }
...
- (void)layoutSubviews
{
    ...
    self.brightLabel.text = @"Brightness";
...
- (void)animateSeekUIViewShow
{
    ...
    [UIView animateWithDuration:0.0 animations:^{
        ...
        AndyLog(@"Fast forward and rewind is displayed");
    }];
}
...
- (void)cameraCaptureButtonClick:(UIButton *)button
{
    ...
    [self captureNoticeShow:^{
        [MBProgressHUD showSuccess:@"The picture captures are completed and saved to the album" shouldAutoRoateAngle:M_PI_2];
    }];
}
...
- (UIButton *)qualityButton
{
    if (_qualityButton == nil)
    {
        ...
        [_qualityButton setTitle:@"HD" forState:UIControlStateNormal];
